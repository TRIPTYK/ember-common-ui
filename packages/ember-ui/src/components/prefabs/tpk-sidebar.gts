		...
		<div class="flex min-h-full flex-col items-start bg-base-200 is-drawer-close:w-14 is-drawer-open:w-64">
			<ul class="menu w-full grow">
				{{#each @sidebarItems as |item|}}
					<li>
						{{#if item.route}}
							<LinkTo @route={{item.route}} class="is-drawer-close:tooltip is-drawer-close:tooltip-right" data-tip={{item.tooltip}}>
								{{#if item.icon}}
						...
						{{else if item.onClick}}
							<button type="button" {{on "click" item.onClick}} class="is-drawer-close:tooltip is-drawer-close:tooltip-right" data-tip={{item.tooltip}}>
								{{#if item.icon}}
									<item.icon class="my-1.5 inline-block size-4" />
								{{/if}}
								<span class="is-drawer-close:hidden">{{item.label}}</span>
