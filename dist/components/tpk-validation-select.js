...
import MandatorySelectLabel from './prefabs/mandatory-select-label.js';
import { precompileTemplate } from '@ember/template-compilation';
import { setComponentTemplate } from '@ember/component';
import { n } from 'decorator-transforms/runtime-esm';

class TpkValidationSelect extends BaseValidation {
  ...
  static {
    setComponentTemplate(precompileTemplate("<div class=\"{{if @disabled \"disabled\"}} tpk-validation-select\" data-has-error=\"{{this.hasError}}\" {{!-- @glint-expect-error --}} anchorScrollUp={{@validationField}} ...attributes>\n  <TpkSelectComponent @multiple={{@multiple}} @onChange={{this.onChange}} @label={{@label}} @options={{@options}} @placeholder={{@placeholder}} @selected={{this.value}} @allowClear={{@allowClear}} @renderInPlace={{@renderInPlace}} @labelComponent={{if @labelComponent @labelComponent (component MandatorySelectLabel mandatory=this.mandatory)}} @selectedItemComponent={{@selectedItemComponent}} @placeholderComponent={{@placeholderComponent}} @searchEnabled={{@searchEnabled}} @searchField={{@searchField}} @searchPlaceholder={{@searchPlaceholder}} @searchMessage={{@searchMessage}} @search={{@search}} @onKeyDown={{@onKeyDown}} @disabled={{@disabled}} @initiallyOpened={{@initiallyOpened}} @loadingMessage={{@loadingMessage}} @noMatchesMessage={{@noMatchesMessage}} as |I|>\n    {{yield (hash Option=I.Option errors=this.errors hasError=this.hasError firstError=this.firstError mandatory=this.mandatory)}}\n  </TpkSelectComponent>\n</div>", {
      strictMode: true,
      scope: () => ({
        TpkSelectComponent,
