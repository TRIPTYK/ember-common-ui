...
function _throwSelectorError(selector, functionName) {
  throw new Error(`${functionName}() - No input was found using selector '${stringifySelector(selector)}'`);
}
function stringifySelector(selector) {
  return selector instanceof HTMLElement ? `<${selector.tagName.toLowerCase()} id="${selector.id}" class="${selector.className}">` : selector;
}
